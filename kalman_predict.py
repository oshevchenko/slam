...
class ExtendedKalmanFilter:
    ...
    def dg_dstate(state, control, w):
        ...
        if r != l:
            ...
            row11 = 1
            row12 = 0
            row13 = (R + w/2)*(cos(theta + alpha) - cos(theta))

            row21 = 0
            row22 = 1
            row23 = (R + w/2)*(sin(theta + alpha) - sin(theta))
            ...
            m = array([[row11, row12, row13], [row21, row22, row23], [row31, row32, row33]])  # Replace this.
    ...
    def predict(self, control):
        ...
        left, right = control
        sigma_l_control = (self.control_motion_factor*left)**2 + (self.control_turn_factor*(left-right))**2
        sigma_r_control = (self.control_motion_factor*right)**2 + (self.control_turn_factor*(left-right))**2

        control_covariance = diag([sigma_l_control, sigma_r_control])

        dg_dcontrol = self.dg_dcontrol(self.state, control, self.robot_width)
        dg_dcontrol_t = transpose(dg_dcontrol)
        R = dot(dg_dcontrol, control_covariance)
        R = dot(R, dg_dcontrol_t)

        dg_dstate = self.dg_dstate(self.state, control, self.robot_width)
        dg_dstate_t = transpose(dg_dstate)

        self.covariance = dot(dg_dstate, self.covariance)
        self.covariance = dot(self.covariance, dg_dstate_t)

        self.covariance = self.covariance + R
...
if __name__ == '__main__':
    ...
    # logfile.read("/home/hd62user/workspace/SLAM/my_unit_f/gym_5/output")
